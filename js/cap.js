// ── Salary Cap Module ────────────────────────────────────────
// Initialized on demand when the Cap tab is first opened.
// Shares Firebase db, leagueId(), username() from app.js context.
...
function subscribeRosters() {
  ...
  rosterRef().on('value', snap => {
    ...
    if (fbData && Object.keys(fbData).length > 0) {
      DATA = fbData;
      document.getElementById('cap-last-upd').textContent = 'Live data';
    } else {
      DATA = JSON.parse(JSON.stringify(FALLBACK));
      document.getElementById('cap-last-upd').textContent = 'Built-in data';
    }
    document.getElementById('cap-loading').style.display = 'none';
    document.getElementById('cap-app').style.display = '';
    if (isComm()) {
      document.getElementById('cap-comm-tab').style.display = '';
      const dd = document.getElementById('cap-dd');
...
function initWithFallback() {
  ...
  document.getElementById('cap-loading').style.display = 'none';
  document.getElementById('cap-app').style.display = '';
  if (isComm()) document.getElementById('cap-comm-tab').style.display = '';
  renderTab(tab);
}
...
// ── Tab switcher ──────────────────────────────────────────────
function setTab(t) { if(typeof setCapTab==="function"){setCapTab(t);return;}
  tab = t;
  ['overview','allplayers','compare','toppaid','taxi','commish'].forEach(n => {
    document.getElementById('tab-'+n).style.display = n===t?'':'none';
  });
  document.querySelectorAll('[data-captab]').forEach(b =>
    b.classList.toggle('active', b.dataset.captab === t)
  );
  const dd = document.getElementById('cap-dd');
  if (dd) dd.value = t;
  renderTab(t);
...
// ── Public init called when Cap tab is activated ────────────
window.Cap.init = function() {
  if (window.Cap._initialized) return;
  window.Cap._initialized = true;
  subscribeRosters();
};

window.Cap.refresh = function() {
  if (typeof subscribeRosters === 'function') subscribeRosters();
};
